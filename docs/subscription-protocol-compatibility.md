...
| Surge | `surge` | Shadowsocks, VMess, Trojan, Hysteria2, AnyTLS, SOCKS, HTTP | No VLESS or TUIC output. Hysteria2 requires Surge 5.8.0+, AnyTLS requires 5.17.0+. HY2 Salamander obfs emits `salamander-password`; port hopping emits `port-hopping` and `port-hopping-interval`. | Profile target for Surge. |
